...
const DepressionTest = () => {
  ...
  const write_data = async (answers, userName = null, userEmail = null, existingSubmissionId = null) => {
    const db = getDatabase(app);
    
    // Calculate the score from the answers
    const score = answers.reduce((sum, item) => {
      // Find the corresponding question and option
      const question = quizData.find(q => q.index === (item.question + 1) || q.index === undefined);
      if (!question) return sum;
      
      // If there's an answer, find its score
      if (item.answer) {
        const option = question.options.find(opt => opt.text === item.answer);
        return sum + (option ? option.score : 0);
      }
      return sum;
    }, 0);
    
    // If we have an existing submission ID and user info, update that entry
    if (existingSubmissionId && userName && userEmail) {
      try {
        // Update only the user information in the existing entry
        await update(ref(db, `assessments/${existingSubmissionId}`), {
          user: {
            name: userName,
            email: userEmail
          },
    ...
    // If no existing submission or creating a new entry
    // Create a new submission ID
    const timestamp = new Date().getTime(); // Get current timestamp in milliseconds
    const uniqueId = uuidv4().slice(0, 8); // Get first 8 chars of UUID for uniqueness
    const submissionId = `${timestamp}-${uniqueId}`;
  
    
    // Prepare the data structure for storage
    const flatDataToStore = {
      submissionId: submissionId,
      timestamp: new Date().toISOString(),
      score: score
    };
    
    // Add user info if provided with the initial submission
    if (userName && userEmail) {
      flatDataToStore.user = {
        name: userName,
        email: userEmail,
      };
    }
    
    // Add responses as direct properties
    answers.forEach((answer, index) => {
      flatDataToStore[`q${index+1}`] = {
    ...
    try {
      await set(ref(db, `assessments/${submissionId}`), flatDataToStore);
      
      // Return the ID so it can be stored in the session/local storage
      return submissionId;
    } catch (error) {
  ...
  const handleAnswerSelect = (option, index) => {
    ...
    const data = newAnswers.map((answer, currentQuestion) => ({
      question: currentQuestion,
      answer: answer || null, // Handle empty answers
  }));
    setAnswers(newAnswers);
    
    if (newAnswers.filter(Boolean).length === 2) {
  ...
  const handleSubmit = (e) => {
    ...
    // Prepare the answer data (same format as in handleAnswerSelect)
    const data = answers.map((answer, questionIndex) => ({
      question: questionIndex,
      answer: answer || null,
    }));
    
    // Call write_data with user info
    write_data(data, name, email);
